...
import DotErc20Contract, { gasLimit} from './Doterc20Contract';
...
function Main (props) {
  ...
  const getTotalSupply = async () => {
    const erc20supply = await doterc20Contract.query.totalSupply();
    return erc20supply.output.toNumber();
  } 

  const transfer = () => {
    doterc20Contract.tx.transfer(0, -1, accountSelected, formAmount).signAndSend(accountPair, async (result) => {
